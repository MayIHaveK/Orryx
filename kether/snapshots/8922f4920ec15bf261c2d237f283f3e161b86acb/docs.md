...
> `arcartx sound send <STRING> *category [STRING(master)] *distance/distOrRoll/volume [INT(16)] *pitch [FLOAT(1.0)] *keepTime/duration [INT(5000)] *they [CONTAINER(@self)]`
...
| distance/distOrRoll/volume | INT | true | 衰减距离 |
| pitch | FLOAT | true | 音调 |
| keepTime/duration | INT | true | 保持时间（毫秒） |
| they | CONTAINER | true | 可听玩家 |
...
# MayDMZAnimation 附属语句

## maydmz

### MayDMZAnimation 可用状态（公有语句）

> `maydmz available`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| available | SYMBOL | false | 可用状态标识符 |

| 返回值类型 | 描述 |
|-----------|------|
| BOOLEAN | API 服务当前是否可用 |

> 仅在安装并激活 MayDMZAnimation 时生效；缺失时安全降级。
...
> `maydmz action stop *channel [STRING] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| action | SYMBOL | false | 动作标识符 |
| stop | SYMBOL | false | 停止标识符 |
| channel | STRING | true | 执行通道 |
| they | CONTAINER | true | 目标玩家 |
...
```yaml
maydmz action stop channel "upper_body" they @self
```

### 停止直接播放（公有语句）

> `maydmz playback stop *transition [FLOAT(4.0)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| playback | SYMBOL | false | 播放标识符 |
| stop | SYMBOL | false | 停止动作 |
| transition | FLOAT | true | 过渡 tick |
| they | CONTAINER | true | 目标玩家 |
...
> `maydmz combo assign <STRING> *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| combo | SYMBOL | false | 连击标识符 |
| assign | SYMBOL | false | 分配标识符 |
| 无 | STRING | false | 连击 ID |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到分配结果的映射 |
...
> `maydmz combo compare-and-set <STRING> <STRING> *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| combo | SYMBOL | false | 连击标识符 |
| compare-and-set | SYMBOL | false | 原子替换标识符 |
| 无 | STRING | false | 预期的当前连击 ID；保留值 "<empty>" 表示无指派 |
| 无 | STRING | false | 新的连击 ID；保留值 "<empty>" 表示清除 |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到替换结果的映射；并发变化返回 conflict |
...
> `maydmz action signal <STRING> *channel [STRING] *they [CONTAINER(@self)]`
...
> `maydmz action cancel <STRING> *channel [STRING] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| action | SYMBOL | false | 动作标识符 |
| cancel | SYMBOL | false | 取消标识符 |
| 无 | STRING | false | 取消原因 |
| channel | STRING | true | 执行通道 |
| they | CONTAINER | true | 目标玩家 |
...
> `maydmz action start <STRING> *priority [INT] *policy [STRING(replace)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| action | SYMBOL | false | 动作标识符 |
| start | SYMBOL | false | 启动标识符 |
| 无 | STRING | false | 动作 ID |
| priority | INT | true | 抢占优先级 |
| policy | STRING | true | 抢占策略 |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到启动结果的映射 |

> 仅在安装并激活 MayDMZAnimation 时生效；缺失时安全降级。

```yaml
maydmz action start "maydmz:rapid_tap_demo" they @self
maydmz action start "maydmz:skill_attack" priority 100 policy replace they @self
```

### 按实例号停止直接播放（公有语句）

> `maydmz playback stop-instance <LONG> *transition [FLOAT(4.0)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| playback | SYMBOL | false | 播放标识符 |
| stop-instance | SYMBOL | false | 实例停止动作 |
| 无 | LONG | false | 播放实例号 |
| transition | FLOAT | true | 过渡 tick |
...
```yaml
maydmz playback stop-instance &instance transition 4.0
```

### 播放动画并返回实例号（公有语句）

> `maydmz playback play-handle <STRING> *mode [STRING(once)] *speed [FLOAT(1.0)] *duration [INT(200)] *transition [FLOAT(4.0)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| playback | SYMBOL | false | 播放标识符 |
| play-handle | SYMBOL | false | 句柄播放动作 |
| 无 | STRING | false | 动画 ID |
| mode | STRING | true | 播放模式 |
| speed | FLOAT | true | 播放速度 |
| duration | INT | true | 最长 tick |
| transition | FLOAT | true | 过渡 tick |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到播放实例号的映射 |

> 仅在安装并激活 MayDMZAnimation 时生效；缺失时安全降级。

```yaml
maydmz playback play-handle "base.fusion_dance_left" mode loop they @self
```

### 查询主动连击（公有语句）

> `maydmz combo current *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| combo | SYMBOL | false | 连击标识符 |
| current | SYMBOL | false | 查询标识符 |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到当前连击 ID 的映射；未分配时为空字符串 |
...
> `maydmz action exists <STRING>`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| action | SYMBOL | false | 动作标识符 |
| exists | SYMBOL | false | 存在标识符 |
| 无 | STRING | false | 动作 ID |

| 返回值类型 | 描述 |
|-----------|------|
| BOOLEAN | 动作目录是否包含该 ID |

> 仅在安装并激活 MayDMZAnimation 时生效；缺失时安全降级。

```yaml
maydmz action exists "maydmz:rapid_tap_demo"
```

### 查询运行动作（公有语句）

> `maydmz action running *channel [STRING] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| action | SYMBOL | false | 动作标识符 |
| running | SYMBOL | false | 运行标识符 |
| channel | STRING | true | 执行通道 |
| they | CONTAINER | true | 目标玩家 |
...
```yaml
maydmz action running they @self
```

### 查询连击是否存在（公有语句）

> `maydmz combo exists <STRING>`
...
| 返回值类型 | 描述 |
|-----------|------|
| BOOLEAN | 连击目录是否包含该 ID |

> 仅在安装并激活 MayDMZAnimation 时生效；缺失时安全降级。

```yaml
maydmz combo exists "maydmz:rapid_tap_demo"
```

### 清除主动连击（公有语句）

> `maydmz combo clear *they [CONTAINER(@self)]`
...
> `maydmz playback play <STRING> *mode [STRING(once)] *speed [FLOAT(1.0)] *duration [INT(200)] *transition [FLOAT(4.0)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| playback | SYMBOL | false | 播放标识符 |
| play | SYMBOL | false | 播放动作 |
| 无 | STRING | false | 动画 ID |
| mode | STRING | true | 播放模式 |
| speed | FLOAT | true | 播放速度 |
| duration | INT | true | 最长 tick |
...
```yaml
maydmz playback play "base.fusion_dance_left" mode loop duration 200 they @self
```
...
> 仅在安装并激活 MayDMZParticle 时生效；缺失时安全降级。
...
> `maydmzparticle stop-entity *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| stop-entity | SYMBOL | false | 实体停止标识符 |
| they | CONTAINER | true | 目标玩家 |
...
> `maydmzparticle play <STRING> *bone [STRING()] *duration [INT(0)] *offset [STRING(0,0,0)] *rotation [STRING(0,0,0)] *scale [STRING(1,1,1)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| play | SYMBOL | false | 播放标识符 |
| 无 | STRING | false | 效果 ID |
| bone | STRING | true | 骨骼或 socket |
| duration | INT | true | 持续 tick；0 表示由效果或显式停止决定 |
| offset | STRING | true | 局部偏移 x,y,z |
| rotation | STRING | true | 欧拉角 pitch,yaw,roll |
| scale | STRING | true | 缩放 x,y,z |
| they | CONTAINER | true | 目标玩家 |

| 返回值类型 | 描述 |
|-----------|------|
| MAP | 玩家 UUID 到播放句柄 UUID 的映射 |
...
> `maydmzparticle play-at <STRING> *duration [INT(0)] *offset [STRING(0,0,0)] *rotation [STRING(0,0,0)] *scale [STRING(1,1,1)] *they [CONTAINER(@self)]`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| play-at | SYMBOL | false | 世界播放标识符 |
| 无 | STRING | false | 效果 ID |
| duration | INT | true | 持续 tick |
| offset | STRING | true | 局部偏移 x,y,z |
| rotation | STRING | true | 欧拉角 pitch,yaw,roll |
| scale | STRING | true | 缩放 x,y,z |
| they | CONTAINER | true | 位置来源玩家 |
...
> `maydmzparticle stop <STRING>`

| 先导词 | 类型 | 可选 | 描述 |
|--------|------|------|------|
| stop | SYMBOL | false | 停止标识符 |
| 无 | STRING | false | 播放句柄 UUID |
...
> 只保留队内人员，或只保留队外人员；未安装 DungeonPlus 时队伍为空
